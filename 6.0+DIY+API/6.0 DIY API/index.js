...
app.get("/random",(req,res)=>{
  ...
  res.json(jokes[randid]);
});
//2. GET a specific joke
app.get("/jokes/:id",(req,res)=>{
  const id=parseInt(req.params.id);
  const joke=jokes.find((jokes)=>jokes.id===id);
  res.json(joke);
});
//3. GET a jokes by filtering on the joke type
app.get("/filter",(req,res)=>{
  const type=req.query.type;
  const filjoke=jokes.filter((jokes)=>jokes.jokeType==type);
  res.json(filjoke);
})
//4. POST a new joke
...
app.put("/jokes/:id",(req,res)=>{
  ...
  const searchind = jokes.findIndex((jokes)=>jokes.id===id);
  jokes[searchind]=repjoke;
  res.json(repjoke);
});
//6. PATCH a joke
